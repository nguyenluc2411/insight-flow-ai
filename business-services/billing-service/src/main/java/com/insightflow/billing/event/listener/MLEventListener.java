...
public class MLEventListener {
    ...
    @KafkaListener(topics = "ml.forecast.generated", groupId = "billing-service-events")
    public void onForecastGenerated(ConsumerRecord<String, String> record, Acknowledgment ack) {
        try {
            ...
            usageTrackingService.incrementForecastsExecuted(tenantId);
            log.debug("Incremented forecasts_executed for tenantId={}", tenantId);
        } catch (Exception e) {
            log.error("Failed to process ml.forecast.generated event: {}", e.getMessage());
        } finally {
            ack.acknowledge();
        }
    }
